...
function setup() {
  ...
  startButton.size(canW/4, canH/7)
...
function draw() {
  ...
  imageMode(CENTER)
  scrBackg = image(roadImg, canW/2, BackgY, canW * 1.4, canH * (5/3));
  scrBackg2 = image(roadImg, canW/2, BackgY2 - 800, canW * 1.4, canH * (5/3));

  BackgY += backgSpeed;
  BackgY2 += backgSpeed;

  if(BackgY > windowWidth - 100){
    BackgY = 380;
  }

  if(BackgY2 > windowWidth - 100){
    BackgY2 = 380;
  }
  ...
  fill('black')
  rect((windowWidth - 1200)/2, canH/120, canW * (4/5), canH/32);
  ...
  fill('black');
  strokeWeight(5);
  rect((windowWidth - 1200)/2, canH/20, canW/(6/5), canH/60)

  fill('red');
  strokeWeight(0);
  if(playerHealth > 0)
  rect((windowWidth - 1200)/2, canH/20, (playerHealth/playerMaxHealth) * canW/(6/5), canH/60)

  fill('black');
  strokeWeight(5);
  rect(((windowWidth - 1200)/2) + canW/(24/15), canH/(120/9), canW/(24/5), canH/60)

  fill('Grey');
  strokeWeight(0);
  if(playerFuel > 0)
  rect(((windowWidth - 1200)/2) + canW/(24/15), canH/(120/9), (playerFuel/playerMaxFuel) * canW/(24/5), canH/60)
...
if(isTouching(player, truck4)){
  ...
  drawSprites();
    ...
    fill('yellow')
    textSize(Math.floor(canArea/18000))
    titleText = text('Car Crashers', (canW - 300)/2, canH/4)
    textSize(Math.floor(canArea/38000))
    msText = text('High Score: ' + metersSaved + 'm', (canW)/2.5, (canH / (4/3)))
    startButton.show()
